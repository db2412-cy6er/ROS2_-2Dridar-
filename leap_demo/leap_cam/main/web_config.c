...
// ================= P0 时间同步：录制期间冻结 Camera->ROS 映射 =================
static esp_err_t sync_hold_handler(httpd_req_t *req)
{
    camera_time_sync_set_hold(true);
    httpd_resp_set_type(req, "application/json; charset=utf-8");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    return httpd_resp_sendstr(req, "{\"ok\":true,\"held\":true}\n");
}

static esp_err_t sync_release_handler(httpd_req_t *req)
{
    camera_time_sync_set_hold(false);
    httpd_resp_set_type(req, "application/json; charset=utf-8");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    return httpd_resp_sendstr(req, "{\"ok\":true,\"held\":false}\n");
}

static esp_err_t sync_status_handler(httpd_req_t *req)
{
    const bool held = camera_time_sync_is_held();
    int64_t rosoff_ns = 0;
    const bool valid = camera_time_sync_get_rosoff(&rosoff_ns);

    char json[160];
    int len = snprintf(
        json,
        sizeof(json),
        "{\"ok\":true,\"held\":%s,\"rosoff_valid\":%s,\"rosoff_ns\":%lld}\n",
        held ? "true" : "false",
        valid ? "true" : "false",
        (long long)rosoff_ns
    );
...
httpd_handle_t start_config_webserver(void)
{
    ...
    httpd_uri_t sync_hold_uri = {
        .uri = "/api/sync/hold",
        .method = HTTP_GET,
        .handler = sync_hold_handler,
        .user_ctx = NULL,
    };

    httpd_uri_t sync_release_uri = {
        .uri = "/api/sync/release",
        .method = HTTP_GET,
        .handler = sync_release_handler,
        .user_ctx = NULL,
    };

    httpd_uri_t sync_status_uri = {
        .uri = "/api/sync/status",
        .method = HTTP_GET,
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server, &sync_hold_uri));
    ESP_ERROR_CHECK(httpd_register_uri_handler(server, &sync_release_uri));
    ESP_ERROR_CHECK(httpd_register_uri_handler(server, &sync_status_uri));
