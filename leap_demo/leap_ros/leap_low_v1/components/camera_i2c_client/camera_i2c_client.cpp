...
// ============ P0 时间同步常量 ============
constexpr size_t kTimeSyncAttempts = 3;
constexpr TickType_t kTimeSyncReadDelayMs = pdMS_TO_TICKS(5);
constexpr TickType_t kTimeSyncPeriod = pdMS_TO_TICKS(10000);
// ==========================================
...
// ============ P0 时间同步状态 ============
// ros_delta_ns = rmw_uros_epoch_nanos() - main_esp_timer*1000，
// 由 microros 任务写入、camera_task 读取（用 s_client_mutex 保护）。
bool s_ros_offset_valid = false;
int64_t s_ros_delta_ns = 0;
// =========================================
...
// ================= P0 时间同步：内部实现 =================
// 前提：调用方已持有 s_client_mutex。

void get_ros_offset_locked(bool *valid, int64_t *ros_delta_ns) {
    if (valid != nullptr) {
        *valid = s_ros_offset_valid;
    }
    if (ros_delta_ns != nullptr) {
        *ros_delta_ns = s_ros_delta_ns;
    }
}

int64_t parse_sync_timestamp_locked(const uint8_t *raw, size_t raw_len) {
    char json[128] = {};
    if (!normalize_json_payload(raw, raw_len, json, sizeof(json))) {
        return INT64_MIN;
    }
    ...
    cJSON *item = cJSON_GetObjectItemCaseSensitive(root, "ts");
    int64_t ts = INT64_MIN;
    if (item != nullptr && cJSON_IsNumber(item)) {
        ts = static_cast<int64_t>(cJSON_GetNumberValue(item));
    }
    cJSON_Delete(root);
    return ts;
}

// 主控主动发起一轮 SYNC：写 "SYNC" 命令，摄像头 RX-ISR 在写入结束瞬间打
// t_cam(esp_timer)，随后主控读回。选“写事务耗时最短”的一组估计
// off_cam_main_us = t_write_end(main) - t_cam(camera)。
esp_err_t run_time_sync_locked() {
    if (!s_initialized) {
        return ESP_ERR_INVALID_STATE;
    }

    bool ros_valid = false;
    int64_t ros_delta_ns = 0;
    get_ros_offset_locked(&ros_valid, &ros_delta_ns);
    if (!ros_valid) {
        // micro-ROS epoch 尚未同步好时先不写 ROSOFF（等 microros 恢复）。
        return ESP_OK;
    }

    bool have_sample = false;
    uint64_t best_write_dur_us = UINT64_MAX;
    int64_t best_offset_us = 0;
    for (size_t i = 0; i < kTimeSyncAttempts; ++i) {
        const uint64_t t0 = esp_timer_get_time();
        if (i2c_write_command("SYNC") != ESP_OK) {
            continue;
        }
        const uint64_t t_write_end = esp_timer_get_time();
        // 等摄像头从机任务把带 ts 的响应加载到 TX buffer。
        vTaskDelay(kTimeSyncReadDelayMs);

        uint8_t raw[CAMERA_I2C_STATUS_JSON_MAX] = {};
        if (i2c_read_raw(raw, sizeof(raw)) != ESP_OK) {
            continue;
        }
        const int64_t ts_cam_us = parse_sync_timestamp_locked(raw, sizeof(raw));
        if (ts_cam_us == INT64_MIN) {
            continue;
        }

        const uint64_t write_dur_us = t_write_end - t0;
        if (!have_sample || write_dur_us < best_write_dur_us) {
            have_sample = true;
            best_write_dur_us = write_dur_us;
            best_offset_us = static_cast<int64_t>(t_write_end) - ts_cam_us;
        }
    }

    if (!have_sample) {
        ESP_LOGW(kTag, "Time sync failed: no valid SYNC samples");
        return ESP_FAIL;
    }

    // ROSOFF_ns 使 camera_local_us*1000 + ROSOFF_ns ≈ ROS epoch。
    const int64_t rosoff_ns = best_offset_us * 1000LL + ros_delta_ns;
    char command[64] = {};
    snprintf(command, sizeof(command), "ROSOFF %lld", static_cast<long long>(rosoff_ns));

    const esp_err_t err = i2c_write_command(command);
    ESP_LOGI(
        kTag,
        "Time sync: off_cam_main_us=%lld rosoff_ns=%lld (write_dur_us=%llu, best of %u) err=%s",
        static_cast<long long>(best_offset_us),
        static_cast<long long>(rosoff_ns),
        static_cast<unsigned long long>(best_write_dur_us),
        static_cast<unsigned>(kTimeSyncAttempts),
        esp_err_to_name(err));
    return err;
}
...
esp_err_t camera_i2c_client_set_ros_offset(int64_t ros_delta_ns) {
    if (!ensure_mutex()) {
        return ESP_ERR_NO_MEM;
    }
    if (xSemaphoreTake(s_client_mutex, kClientMutexTimeout) != pdTRUE) {
    ...
    s_ros_delta_ns = ros_delta_ns;
    s_ros_offset_valid = true;
    xSemaphoreGive(s_client_mutex);
    return ESP_OK;
}

void camera_i2c_client_clear_ros_offset(void) {
    if (s_client_mutex == nullptr) {
        s_ros_offset_valid = false;
        return;
    }
    if (xSemaphoreTake(s_client_mutex, kClientMutexTimeout) != pdTRUE) {
        return;
...
esp_err_t camera_i2c_client_try_set_ros_offset(int64_t ros_delta_ns) {
    if (s_client_mutex == nullptr) {
        return ESP_ERR_INVALID_STATE;
    }
...
void camera_i2c_client_try_clear_ros_offset(void) {
    if (s_client_mutex == nullptr) {
        s_ros_offset_valid = false;
        return;
    }
...
esp_err_t camera_i2c_client_run_time_sync(void) {
    if (!ensure_mutex()) {
        return ESP_ERR_NO_MEM;
    }
    if (xSemaphoreTake(s_client_mutex, kClientMutexTimeout) != pdTRUE) {
    ...
    const esp_err_t err = run_time_sync_locked();
    xSemaphoreGive(s_client_mutex);
    return err;
}
...
void camera_task(void *arg) {
    ...
    while (true) {
        ...
        // P0 时间同步：每 ~3s 发起一轮 Camera↔Main I2C 同步，向摄像头刷新 ROSOFF。
        const TickType_t tick_now = xTaskGetTickCount();
        if (last_time_sync == 0 || (tick_now - last_time_sync) >= kTimeSyncPeriod) {
            last_time_sync = tick_now;
            (void)camera_i2c_client_run_time_sync();
        }
    }
}
