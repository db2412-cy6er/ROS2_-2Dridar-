...
// ================= P0 时间同步 =================
// microros 任务在 epoch 已同步时把 ros_delta_ns(=rmw_uros_epoch_nanos()
// - main esp_timer*1000) 灌进来；camera 时间同步任务据此合成发给摄像头的
// "ROSOFF <ns>"（camera_local_us*1000 + rosoff_ns ≈ ROS epoch）。
esp_err_t camera_i2c_client_set_ros_offset(int64_t ros_delta_ns);
void camera_i2c_client_clear_ros_offset(void);
// P0.1: 非阻塞(trylock)变体，拿不到 s_client_mutex 立即返回，不阻塞发布任务。
esp_err_t camera_i2c_client_try_set_ros_offset(int64_t ros_delta_ns);
void camera_i2c_client_try_clear_ros_offset(void);
// 主动发起一轮 Camera↔Main I2C 时间同步（多组 SYNC 取最优），成功后向
// 摄像头写入新的 ROSOFF（摄像头处于 hold 时会忽略，保证录制期间映射不变）。
