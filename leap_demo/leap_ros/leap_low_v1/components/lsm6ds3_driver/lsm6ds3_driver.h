...
class Lsm6ds3Imu {
 public:
  ...
  // P0 时间同步：返回最近一次 Update() 中“LSM6DS3 原始寄存器批量读取完成瞬间”的
  // esp_timer 时间(us)。这比在调用方采样结束后再打点更贴近真正读取时刻。
  uint64_t GetSampleUsUs() const { return sample_us_; }
  // 获取欧拉角 (度)
 ...
 private:
  ...
  uint64_t last_us_;
  // P0 时间同步：记录最近一次原始寄存器读取完成瞬间的 esp_timer 时间(us)。
  uint64_t sample_us_;
