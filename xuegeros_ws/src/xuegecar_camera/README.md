...
`frame_id=camera_link`. It also publishes raw RGB frames on `/camera/image_raw`
by default, which is required by the `xuegecar_yolo` semantic detection
node (P2). If no consumer needs the raw topic, set `publish_raw:=false`
to save ROS bandwidth and avoid stuttering over Wi-Fi.
